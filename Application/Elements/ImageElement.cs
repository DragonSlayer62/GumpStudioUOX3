...
namespace GumpStudio.Elements
{
	...
	public class ImageElement : BaseElement, ICSharpExportable
	{
		...
		public override string Type => "AddGump";
		...
		public string ToCSharpString()
		{
			if (!Name.StartsWith(Type))
			{
				if (Hue?.Index > 0)
				{
					return $"uox3gump.AddGumpColor({X}, {Y}, {GumpID}, {Hue}); // {Name}";
				}

				return $"uox3gump.AddGump({X}, {Y}, {GumpID}); // {Name}";
			}

			if (Hue?.Index > 0)
			{
				return $"uox3gump.AddGumpColor({X}, {Y}, {GumpID}, {Hue});";
			}

			return $"uox3gump.AddGump({X}, {Y}, {GumpID});";
		}
	}
}
